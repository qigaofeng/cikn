...
#
#             # [batch_size, -1, n_neighbor, 1]  #领域是4维
...
class Aggregator(object):
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        if not avg:
            ...
            user_relation_scores = tf.reduce_mean(user_embeddings * neighbor_relations, axis=-1)
            user_relation_scores_normalized = tf.nn.softmax(user_relation_scores, dim=-1)
           # print(user_relation_scores_normalized) # Tensor("Softmax:0", shape=(128, ?, 8), dtype=float32)
            # result = tf.cond(user_relation_scores_normalized <= 0, lambda: mask.append(user_relation_scores_normalized), lambda: 'pass')
...
class SumAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, user_embeddings,masks):
       ...
       # att_user_item = tf.reduce_mean(self.user_relation_scores_normalized * self_vectors, axis=2)
        # [-1, dim]
        # output1 = tf.reshape(user_embeddings + att_user_item, [-1, self.dim])
        # output1 = tf.nn.dropout(output1, keep_prob=1 - self.dropout)
        # output1 = tf.matmul(output1, self.weights) + self.bias
        ...
        #output1 = tf.reshape(output1, [self.batch_size, -1, self.dim])
        return self.act(output), #self.act(output1)
